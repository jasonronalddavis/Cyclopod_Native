...
const HexapodControl = ({ device }) => {
  ...
  const widthRef = useRef(new Animated.Value(140)).current;
  const heightRef = useRef(new Animated.Value(140)).current;
  ...
  return (
    ...
    <Animated.View
      style={[styles.xrectangle, { width: widthRef }]}
      {...panResponderX.panHandlers}
    >
      <Image source={eyeBar} style={styles.xRectangleImage} />
    </Animated.View>
    <Animated.View
      style={[styles.yRectangle, { height: heightRef }]}
